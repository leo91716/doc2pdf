...
class Writer_Track():
    ...
    def getBasicInfo(self):
        ...
        age=collect1['CreatTime'].year-collect1['DateOfBirth'].year
        if age<7:
            age='-'
        tableList=[
        ...
        ['年齡:',age,'教育年:',collect1['HighestLevelOfEducation'], '學校:']
        ]
    ...
    def getInnerValue(self,data,low,high,getLow,getHigh):
    ...
    def getMoreMeasure(self):
        table2=[x[1:] for x in self.tableList2[1:]]
        ...
        i3=['量尺\n分數']+self.getScaleAndPr('table3', i2[1:],reverse=self.getNormReverse()[1])[0]
        i4=['PR值']+self.getScaleAndPr('table3', i2[1:],reverse=self.getNormReverse()[1])[1]
...
class Writer_Fluent(Writer_Track):  #newScore(data, mean,std, new_mean, new_std)
    ...
    def getBasicMeasureI2(reader):
        ...
        i2=[int(reader.getData(*getDataArg,'情境1_黑點相連')),                           int(reader.getData(*getDataArg,'情境2_白點相連')),          int(reader.getData(*getDataArg,'情境3_黑點和白點互相轉換'))]
        i2end=[sum(i2)]
        i2=i2+i2end
        return i2
    @staticmethod
    ...
    def getMoreMeasure(self):
        ...
        print('getMoreMeasureI2',i2)
        i3=['量尺分數']+self.getScaleAndPr('table3', i2[1:],reverse=self.getNormReverse()[1])[0]
        i4=['PR值']+self.getScaleAndPr('table3', i2[1:],reverse=self.getNormReverse()[1])[1]
        tableList3=[i1,i2,i3,i4]
        self.add_paragraph('')
        self.add_paragraph('衍生測量:')
