...
def chooseSource():
    ...
    old_folder=source_folder
    source_folder = filedialog.askdirectory()
    if not source_folder:
        source_folder =old_folder
    labelsource["text"] = '輸出資料夾:  '+source_folder
    pb["maximum"] = len(glob.glob(source_folder+"\\*.xlsx"))
...
try:
    ...
    btnGo = Button(root,text="執行",command=lambda: go(dest_folder,source_folder))
    ...
    btnsource = Button(root,text="選擇路徑",command=lambda: chooseSource())
    ...
    labeldest["text"] = '輸出資料夾:  '+dest_folder
    labelsource['text']='讀取資料夾   '+source_folder
