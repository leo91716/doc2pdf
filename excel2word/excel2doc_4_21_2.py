...
def choosePath():
    ...
    label["text"] = '輸出資料夾:  '+folder_selected


def go(source_folder):
    i=0
    '''
    ...
    if not os.path.exists('pdf'):
        os.makedirs('pdf')
    '''
    if not os.path.isdir(source_folder+'/word'):
        os.makedirs(source_folder+'/word')
    if not os.path.isdir(source_folder+'/pdf'):
        os.makedirs(source_folder+'/pdf')
    
    for file in glob.glob("*.xlsx"):
        print(source_folder)
        reader=Reader(file)
        excel2doc(reader, file,source_folder)
        i+=1
        pb["value"] = i
        root.update()
...
try:
    ...
    btnsource = Button(root,text="選擇路徑",command=lambda: choosePath())
    btnGo = Button(root,text="執行",command=lambda: go(folder_selected))
    label.pack()       
